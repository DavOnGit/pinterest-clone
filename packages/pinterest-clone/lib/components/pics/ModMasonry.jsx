...
class Masonry extends React.PureComponent {
  ...
  componentDidMount() {
    if (this.props.results) { this.layout(this.props) }
  ...
  layout = (props, rearrange=false) => {
    ...
    const componentName = itemComponent.displayName || itemComponent.type.displayName || itemComponent.type.name

    if (!('getHeightFromProps' in itemComponent) && !('getHeightFromProps' in itemComponent.type)) {
      throw new Error(`Component type ${componentName} does not respond to 'getHeightFromProps'`)
    }

    const heightSelector = itemComponent.getHeightFromProps || itemComponent.type.getHeightFromProps
    const columnSpanSelector = itemComponent.getColumnSpanFromProps || itemComponent.type.getColumnSpanFromProps || defaultColumnSpanSelector
    ...
    const pages = itemsToLayout.reduce((workingPages, itemProps) => {
      ...
      const columnSpan = Math.min(maxColumns, columnSpanSelector(itemProps))
      ...
      const height = heightSelector(itemProps, columnSpan, columnGutter)

      if (isNaN(height)) {
        console.warn( // eslint-disable-line
          `Skipping feed item ${componentName} with props ${JSON.stringify(itemProps)} because "${height}" is not a number.`
        )
        return workingPages
      }
    ...
    const averageHeight = Math.round(
      stagedItems.map(item => item.height).reduce((prev, val) => prev + val, 0) / stagedItems.length
    )
